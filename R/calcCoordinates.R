...
calculateConnections <- function(ped, type = c("siblings","parent-child", "spouses")) {
  ...
  spouse_connections <- ped %>%
    ...
    dplyr::left_join(ped, by = c("spouseID" = "personID"), suffix = c("", "_spouse")) %>%
  ...
  connections <- connections %>%
    dplyr::left_join(mom_connections, by = join_by(personID, momID)) %>%
    dplyr::left_join(dad_connections, by = join_by(personID, dadID)) %>%
    dplyr::left_join(spouse_connections, by = join_by(personID, spouseID))
  ...
  connections <- connections %>%
    dplyr::left_join(parent_midpoints, by = c("dadID", "momID")) %>%
    dplyr::left_join(spouse_midpoints, by = join_by(spouseID))  %>%
    dplyr::left_join(sibling_midpoints, by = join_by(dadID, momID)) %>%
    mutate(
