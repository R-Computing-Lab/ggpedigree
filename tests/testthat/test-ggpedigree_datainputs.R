expect_roundtrip <- function(df,
  ...
  if (expect_warnings == TRUE) {
    expect_no_error(suppressWarnings(ggPedigree(df)))
  } else {
    expect_silent(ggPedigree(df))
...
test_that("num ids; 0 missingid; 1/2", {
  expect_roundtrip(case_num(c(2, 1, 2), 0), NA_id_value = 0)
})
test_that("num ids; 0 missingid; 1/2 + NA", {
  expect_roundtrip(case_num(c(2, 1, NA), 0), NA_id_value = 0)
})
test_that("num ids; 0 missingid; 0/1", {
  expect_roundtrip(case_num(c(1, 0, 1), 0), NA_id_value = 0)
})
test_that("num ids; 0 missingid; 0/1 + NA", {
  expect_roundtrip(case_num(c(1, 0, NA), 0), NA_id_value = 0)
})

test_that("num ids; NA missingid; 0/1", {
  expect_roundtrip(case_num(c(1, 0, 1), NA))
})
test_that("num ids; NA missingid; 0/1 + NA", {
  expect_roundtrip(case_num(c(1, 0, NA), NA))
})
test_that("num ids; NA missingid; 1/2 + NA", {
  expect_roundtrip(case_num(c(2, 1, NA), NA))
})
