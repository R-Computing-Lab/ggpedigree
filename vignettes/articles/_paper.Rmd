...
For standard plotting tasks, `ggpedigree` is self contained: it ships built-in pedagogical pedigrees (e.g., `ASOIAF`, `redsquirrels`) and only requires `ggplot2`. BGmisc remains an optional companion for relatedness-heavy workflows, supplying validation utilities (`checkParentIDs()`, `ped2fam()`) and the additive/mitochondrial matrices consumed by `ggRelatednessMatrix()`.
...
-   Data Standardization and Family Structure Analysis: `ggPedigree()` integrates with network-based functions from BGmisc like `ped2fam()` to organize individuals by family and `checkParentIDs()` to validate pedigree structures. The function handles consanguineous relationships and individuals appearing in multiple pedigree positions.
...
This example shows how to use `ggpedigree` to visualize a pedigree using only package data. The `ASOIAF` dataset includes several fictional families from the Song of Ice and Fire universe.
...
```{r ggpedigree_plot, echo=FALSE, message=FALSE, warning=FALSE}
# Appendix
