...
```{r ggpedigree_plot, fig.dim=c(9.5, 6), message=FALSE, warning=FALSE, include=FALSE}
library(ggpedigree)
library(BGmisc)
library(ggpedigree)
library(tidyverse)
...
m1 <- ggPedigree(df_potter %>% filter(personID %in% c(1:7, 101:104)),
  ...
  config = list(
    ...
    label_text_size = text_size_wfu,#3,
...
result <- m1 + m2 +
  ...
  theme(
    ...
    plot.tag = element_text(family = family_wfu, face = "bold")
  )
# save as a png
ggsave(
