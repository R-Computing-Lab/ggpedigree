...
```{r ggpedigree_plot, message=FALSE, warning=FALSE, include=FALSE}
library(ggpedigree)
library(BGmisc)
library(ggpedigree)
library(tidyverse)
...
m2 <- ggPedigree(df_potter,
  ...
  config = list(
    ...
    focal_fill_high_color = "#9E7E38", # text_color_wfu[1],
    focal_fill_mid_color = "#9E7E38", # text_color_wfu[1],
  ...
  plot.title = element_blank(),
  plot.title.position = "plot",
  text = element_text(family = family_wfu, size = 14, face = "bold")
...
result <- m1 + m2 +
  ...
  theme(
    ...
    plot.tag = element_text(family = family_wfu, size = 28, face = "bold")
  )
# save as a png
ggsave(
