...
You can also highlight multiple individuals by creating separate plots for each focal person and combining them using `patchwork` or similar packages. Below, we highlight both Harry Potter and Ginny Weasley in separate plots, then combine them side by side.
...
You can also highlight all the distinct maternal relatives of a person by setting `focal_fill_component = "matID"`. This will group all individuals that share the same mitochondrial line and shade them according their group membership. The `focal_fill_method` can be set to "viridis_d" to use a discrete color scale, which is particularly useful for large pedigrees with many individuals.
